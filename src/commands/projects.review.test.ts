...
import { projectsReviewApproveCommand, projectsReviewRejectCommand } from "./projects.review.js";
...
`;
    ...
    await expect(projectsReviewApproveCommand({ taskId: "TASK-001", projectDir })).rejects.toThrow(
      /not found in Review/,
    );
  });
});
...
`;
    ...
    await expect(projectsReviewRejectCommand({ taskId: "TASK-001", projectDir })).rejects.toThrow(
      /not found in Review/,
    );
  });
});
